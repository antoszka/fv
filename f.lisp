...
;;;
;;; This is my personal invoicing program, might only be useful in the
;;; Polish VAT-invoice area.
;;;
...
;;;
;;; load our code for translating number to Polish:
;;;

(defvar *program-directory* (make-pathname
			     :directory '(:absolute
			       "home"
			       "antoni"
			       "fv")))

(load (merge-pathnames *program-directory*
...
(defun make-client (&key name address postcode (city "Warszawa")
  ...
  (list
   ...
   :email        email
   :nick         nick
   :payment-cash payment-cash ; ← nil (default) means we want a bank transfennnr
...
(defun make-invoice (&key client items payment-cash
  ...
  (let* ((universal-time (get-universal-time))
	 ...
	 (invoice-id     (format nil "FV-~d/~d/~d" invoice-number invoice-month invoice-year))) ; this is autogenerated for consistency
...
(defun print-invoice (invoice)
  ...
  (let* ((invoice-id        (getf invoice :id))
    ...
    ;; DEBUG:
    (format nil
	    "~%FILENAME:~10t~a~%ID:~10t~a~%DATE:~10t~a~%NAME:~10t~a~%ADDRESS:~10t~a~%POSTCODE:~10t~a~%NIP:~10t~a~%~%ITEMS:~%~{~a:~10t~a~%~}~%"
	    output-filename invoice-id invoice-date-full buyer-name buyer-address buyer-postcode buyer-nip item-list)
    ;; END DEBUG
    (emb:register-emb "template" (merge-pathnames *program-directory* (make-pathname :name "emb-template"
										     :type "tex")))
